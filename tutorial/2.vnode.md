...
its simplest the ring is an zero to 2^160-1 integer hashspace. This space is
split into `partitions`, each an equal range of the hash space.  A
Partition has an ID or `index` that is the maximum hashed key it is
responsible for.  We use a `hash function` to assign work (or
storage). We hash a `key` to an integer on the ring, and the next
...
information about what node is repsonsible for an index or partition
...
Riak Core starts processes for the `partitions` hosted by a node but
...
making a real database, they're hard(TM). Later maybe we'll refactor to
use [leveldb](https://github.com/basho/leveldb) or
...
[SyncFree github account](https://github.com/SyncFree/crdtdb). If you
want to play along please run

    make relclean
    make stage

This gets rid of any `release` you already made, and links the `ebin`
directories from into a `release`. Don't worry about what that means,
what it _does_ is allow you to run `./rebar compile` and then just
start you release node and it will pick up you changed code.
...
don't want to make this an erlang tutorial too, so I may skip over
things in a confusing way, if you get that sensation, please mail the
list.
...
callback function with the argument we gave it above. Let's do that now.
...
Lots of oportunity to refactor the common code between `get` and `put` but `meh`. Later.
...
compiles, you should be able to make a node with `make stage` or `make
rel` and start it up:
...
Great. No lets "concurrently" write to that same key:
...
Note the different actor, and the empty starting set. What happens if we write a counter to `<<"foo">>`?
    ...
    {ok, C} = riak_dt_pncounter:update({increment, 100}, actor1, riak_dt_pncounter:new()).
    {ok,[{actor1,100,0}]}
...
Next time we'll cover prefernce lists, replication, quoroms, and
